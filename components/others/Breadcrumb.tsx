    ...
    <nav aria-label="breadcrumb" className="md:flex hidden items-center my-5 gap-3">
      ...
      {crumbs.map((c, i) => (
        <div key={`${c.title}-${i}`} className="flex items-center gap-2">
          ...
          {c.slug ? (
            <Link href={c.slug} title={t(c.title)} className={`text-sm ${i === crumbs.length - 1 ? "text-sitePrimary font-medium" : "hover:text-sitePrimary"}`}>
              {t(c.title)}
            </Link>
          ) : (
            <span className="text-sm text-gray-600">{t(c.title)}</span>
          )}
        </div>
      ))}
    </nav>
