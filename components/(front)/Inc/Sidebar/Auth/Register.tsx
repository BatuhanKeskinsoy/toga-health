...
function Register({ authLoading, setAuth }: IRegisterProps) {
  ...
  const renderValidationIcon = (condition: boolean) =>
    ...
    label: string,
    type: "email" | "search" | "text" | "tel" | "url" | "none" | "numeric" | "decimal",
    autoComplete?: string,
    index?: number
  ) => (
    ...
    <div className="flex flex-col gap-4 w-full h-full">
      ...
      >
        <div className="flex flex-col lg:gap-8 gap-4 w-full lg:h-full h-max overflow-y-auto pr-3">
          {renderInput("firstName", t("Adınız"), "text", "given-name", 0)}
          {renderInput("lastName", t("Soyadınız"), "text", "family-name", 1)}
          ...
          {["password", "passwordConfirm"].map((id, index) => (
            <label htmlFor={id} key={id} className="flex flex-col gap-4 w-full">
              ...
              {id === "passwordConfirm" && <span>{t("Şifreniz (Tekrar)")}</span>}
              <input
                id={id}
                ...
                onChange={handleChange}
                className="bg-white border border-gray-200 focus:border-sitePrimary/50 rounded-lg py-3 px-6 outline-none text-base w-full"
                placeholder={id === "password" ? t("Şifrenizi giriniz") : t("Şifreniz (Tekrar)")}
                autoComplete="off"
                tabIndex={4 + index}
              />
            </label>
          ...
          <div className="flex items-center gap-3 cursor-pointer group">
            ...
            <label
              ...
              className={`transition-all duration-300 -mb-0.5 cursor-pointer select-none ${
                accept ? "text-sitePrimary" : "lg:group-hover:text-sitePrimary"
              }`}
            >
              <p className="text-sm">
                <Link className="font-medium text-gray-800 hover:text-sitePrimary transition-all" href="/">
                  {t("Kullanıcı Sözleşmesi")}
                </Link>{" "}
                <span className="font-normal">{t("ve")}</span>{" "}
                <Link className="font-medium text-gray-800 hover:text-sitePrimary transition-all" href="/">
                  {t("Çerez Aydınlatma Metni")}
                </Link>{" "}
                <span className="font-normal">{t("Okudum, Kabul Ediyorum")}</span>
              </p>
            </label>
          </div>
        </div>
